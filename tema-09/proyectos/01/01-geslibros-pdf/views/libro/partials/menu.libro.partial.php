    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link <?= in_array($_SESSION['role_id'], $GLOBALS['libro']['nuevo']) ? 'active' : 'disabled' ?>"
                ...
                <li class="nav-item">
                    <a class="nav-link <?= in_array($_SESSION['role_id'], $GLOBALS['libro']['pdf']) ? 'active' : 'disabled' ?>"
                        aria-current="page" href="<?= URL ?>libro/pdf/<?= $_SESSION['csrf_token']?>">Mostrar PDF</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle <?= in_array($_SESSION['role_id'], $GLOBALS['libro']['ordenar']) ? 'active' : 'disabled' ?>"
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li><a class="dropdown-item"
                                href="<?= URL ?>libro/ordenar/5/<?= $_SESSION['csrf_token'] ?>">Fecha de Edición</a>
                        </li>
                        <li><a class="dropdown-item"
            ...
            <form class="d-flex" role="search" action="<?= URL ?>libro/filtrar" method="GET">
                ...
                <button
                    class="btn btn-outline-primary <?= in_array($_SESSION['role_id'], $GLOBALS['libro']['filtrar']) ? null : 'disabled' ?>"
